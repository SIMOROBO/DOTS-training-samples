...
public class DistanceField : MonoBehaviour
{
    ...
    public static float GetDistance(float x, float y, float z, out Vector3 normal)
    {
        ...
        else if (instance.model == DistanceFieldModel.SpherePlane)
        {
            ...
            float t = Mathf.Sin(time * 8f) * 0.4f + 0.4f;
        ...
        else if (instance.model == DistanceFieldModel.FigureEight)
        {
            ...
            float wave = Mathf.Cos(angle * flipper * 3f) * 0.5f + 0.5f;
            wave *= wave * 0.5f;
            distance = Mathf.Sqrt(normal.x * normal.x + normal.y * normal.y + normal.z * normal.z) - (0.5f + wave);
        }
        else if (instance.model == DistanceFieldModel.PerlinNoise)
        {
            float perlin = Mathf.PerlinNoise(x * 0.2f, z * 0.2f);
    ...
    private void Update()
    {
        switchTimer += Time.deltaTime * 0.1f;
        if (switchTimer > 1f)
        {
            switchTimer -= 1f;
        ...
        if (preview)
        {
            ...
            for (int i = 0; i < 3000; i++)
            {
                ...
                if (dist < 0f)
                {
                    Debug.DrawRay(new Vector3(x, y, z), Vector3.up * 0.1f, Color.red, 1f);
                }
            }
        }
    }
